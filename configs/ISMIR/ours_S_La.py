...
config['TRAINING_SEQ_LENGTH'] = (10*min(config['ACCEPTED_SAMPLING_RATES']))//config['KERNEL_SIZE_ENCODER']
...
loss = StereoLoss(Loss(nn.L1Loss(),
...
config['VALID_LOSSES']['stereo_loss']  = StereoLoss(Loss(nn.L1Loss(),
                                                          DataType.STFT_MAGNITUDE,
                                                          guard_left=f_guard,
                                                          guard_right=f_guard),
                                                     filter_type=["aw", "lp"])
