...
from common_datatypes import (
    ri2m,
    ri2mp,
    mp2ri,
    spectral_to_time_domain,
    time_domain_to_spectral,
)
from common_datatypes import unfold, reconstruct_from_unfold, get_length
...
class SeparationNet(nn.Module):
    ...
    __constants__ = [
        "input_type",
        "output_type",
    ]  # such that torch.jit.script can pick them up
...
class SuperNet(nn.Module):
    ...
    def __init__(
        ...
        self.net = net
        assert (
            self.net.input_type == self.net.output_type
        )  # assume that input/output type match
        ...
        self.batched_forward = lambda x, m: torch.cat(
            [m(_in) for _in in torch.split(x, self.training_batch_size)], dim=1
        )
    ...
    def _insert_soft_weights(self, p_name: str, p: nn.Parameter, module: nn.Module):
        ...
        uprint(
            f"\t{p_name:>30}: {module.q_params[p_name]} "
    ...
    def _apply_replace_with_quantized_weights(
        self, p_name: str, p: nn.Parameter, module: nn.Module
    ):
        """
    ...
    def replace_with_quantized_weights(self):
        ...
        """
        self._iterate_modules_recursive(
            self, self._apply_replace_with_quantized_weights
        )
    ...
    def quantize(self, q_op, q_bw):
        ...
        uprint(f"Quantizing neural network weights " f"(q_op = {q_op}, q_bw = {q_bw}):")
    ...
    def _convert_to_required_types(self, x: dict, required_types: set):
        ...
        if DataType.TIME_SAMPLES in required_types and DataType.TIME_SAMPLES not in x:
            if DataType.STFT_COMPLEX in x:
                ...
                x[DataType.TIME_SAMPLES] = spectral_to_time_domain(
                    x[DataType.STFT_COMPLEX],
                ...
                x[DataType.TIME_SAMPLES] = spectral_to_time_domain(
                    x[DataType.STFT_MAGNITUDEPHASE],
                ...
                raise ValueError(
                    "Computation of time-domain output not possible"
        ...
        if DataType.STFT_COMPLEX in required_types and DataType.STFT_COMPLEX not in x:
            if DataType.STFT_MAGNITUDEPHASE in x:
                ...
                x[DataType.STFT_COMPLEX] = time_domain_to_spectral(
                    x[DataType.TIME_SAMPLES],
                    DataType.STFT_COMPLEX,
                    self._stft_window,
                    self._stft_fft_size,
                    self._stft_hop_length,
                ...
                raise ValueError(
                    "Computation of STFT real/imaginary output not possible"
                    " as no suitable time-domain or STFT representation was found."
                )

        if (
            DataType.STFT_MAGNITUDEPHASE in required_types
            and DataType.STFT_MAGNITUDEPHASE not in x
        ):
            if DataType.STFT_COMPLEX in x:
                x[DataType.STFT_MAGNITUDEPHASE] = ri2mp(x[DataType.STFT_COMPLEX])
            elif DataType.TIME_SAMPLES in x:
                x[DataType.STFT_MAGNITUDEPHASE] = time_domain_to_spectral(
                    x[DataType.TIME_SAMPLES],
                    DataType.STFT_MAGNITUDEPHASE,
                    self._stft_window,
                    self._stft_fft_size,
                ...
                raise ValueError(
                    "Computation of STFT magnitude/phase output not possible"
                    " as no suitable time-domain or STFT representation was found."
                )

        if (
            DataType.STFT_MAGNITUDE in required_types
            and DataType.STFT_MAGNITUDE not in x
        ):
            if DataType.STFT_MAGNITUDEPHASE in x:
                ...
                x[DataType.STFT_MAGNITUDE] = time_domain_to_spectral(
                    x[DataType.TIME_SAMPLES],
                ...
                raise ValueError(
                    "Computation of STFT magnitude output not possible"
                    " as no suitable time-domain or STFT representation was found."
                )
    ...
    def preprocess(self, x, losses: dict, output_data_type=None):
        ...
        if self.net.input_type in [
            ...
            if self.net.output_type == DataType.STFT_MAGNITUDE and (
                DataType.STFT_COMPLEX in required_output_types
                or DataType.STFT_MAGNITUDEPHASE in required_output_types
                or DataType.TIME_SAMPLES in required_output_types
            ):
        ...
        if self.batched_valid is True and not self.training:
            ...
            x = torch.stack(
                [unfold(_x, **self.unfolding_params) for _x in x.unbind(0)], dim=0
            )
    ...
    def forward(self, x, losses: dict):
        ...
        with amp.autocast(enabled=self.use_amp):
            ...
            # do preprocessing
            x, original_length, required_output_types, _input_type = self.preprocess(
                x, losses
            )
            ...
            if self.net.input_type == DataType.TIME_SAMPLES:
                ...
                if _input_type == self.net.input_type:
                    ...
                    out[DataType.STFT_MAGNITUDEPHASE] = torch.stack(
                        [
                            self.batched_forward(x[0, ..., 0], self.net),
                            x[0, ..., 1].expand(x[1:, ..., 1].shape),
                        ],
                        dim=-1,
                    )
            ...
            if self.batched_valid is True and not self.training:
                for (
                    data_type
                ) in out:  # out/tar sole key is not necessarily self.net.input_type
                    out[data_type] = reconstruct_from_unfold(
                        out[data_type],
                        original_length=original_length,
                        **self.unfolding_params,
                    )
                    tar[data_type] = reconstruct_from_unfold(
                        tar[data_type],
                        original_length=original_length,
                        **self.unfolding_params,
                    )
    ...
    def inference(self, x):
        ...
        with amp.autocast(enabled=self.use_amp):
            ...
            # do preprocessing
            x, original_length, required_output_types, _input_type = self.preprocess(
                x, losses={}, output_data_type=DataType.TIME_SAMPLES
            )
            # perform forward pass through separation network
            out = {}
            if self.net.input_type == DataType.TIME_SAMPLES:
