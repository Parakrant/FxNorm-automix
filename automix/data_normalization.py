...
from automix.utils_data_normalization import (
    get_comp_matching,
    get_eq_matching,
    get_mean_peak,
    lufs_normalize,
...
def get_norm_feature(args_):
    ...
    print(
        f"getting {effect} features for {source_str}- stem {i} of {len(audio_path_)-1} {path}"
    )
    ...
    if all_zeros == False:
        conversion_scale = 1.0 / (1.0 + np.iinfo(audio.dtype).max)
        ...
        if max_db > MIN_DB:
            if effect == "loudness":
                ...
                audio_spec = compute_stft(
                    audio, HOP_LENGTH, FFT_SIZE, np.sqrt(np.hanning(FFT_SIZE + 1)[:-1])
                )
...
def normalize_audio_path(args_):
    ...
    print(
        f"{src} normalizing {effect} for {source_str} - stem {i} of {len(audio_path_)-1} {path}"
    )
    ...
    if src in STEMS:
        ...
        if all_zeros == False:
            ...
            if (
                audio_track.shape[1] == 1
            ):  # Converts mono to stereo with repeated channels
            ...
            assert fs == SR
            conversion_scale = 1.0 / (1.0 + np.iinfo(audio_.dtype).max)
        ...
        if all_zeros == False:
            ...
            tt = (int(t) * (len(audio_path_) - i - 1) / 3600) / (
                CPU_COUNT * len(EFFECTS)
            )
            tt = "{:.2f}".format(tt)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input-folder", help="Path to dataset folder", type=str, required=True
    )
    ...
    if FEATURES_LOAD:
        ...
        raise ValueError(
            f"file to load features or file to save features should be provided"
        )

    NORM_AUDIO = (
        args.normalize
    )  # If True audio is normalized. If false only features calculated.
