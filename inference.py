...
from common_dataprocessing import create_dataset, load_wav, save_wav
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vocals", type=str, default=None, help="Path to vocals")

    parser.add_argument("--bass", type=str, default=None, help="Path to bass")

    parser.add_argument("--drums", type=str, default=None, help="Path to drums")

    parser.add_argument("--other", type=str, default=None, help="Path to other")
    ...
    for k, inp in enumerate(STEMS):
        if audio_path[inp]:
            ...
            if pad_samples:
                _audio = (
                    _audio[0],
                    np.pad(_audio[1], [(pad_samples, 0), (0, 0)], "wrap"),
                )
            ...
            if _samplingrate not in accepted_sampling_rates:
                ...
                conversion_scale = 1.0 / (1.0 + np.iinfo(temp_audio.dtype).max)
                temp_audio = temp_audio.astype(dtype=np.float32) * conversion_scale

                temp_audio = librosa.resample(
                    temp_audio.T, _samplingrate, SR, res_type="kaiser_best"
                ).T

                temp_audio = temp_audio * (1 + np.iinfo(dtype).max)
                if (
                    np.min(temp_audio) < np.iinfo(dtype).min
                    or np.max(temp_audio) > np.iinfo(dtype).max
                ):
                    uprint(f"WARNING: Clipping occurs for {file_path}. when resampling")
                temp_audio = np.clip(
                    temp_audio, np.iinfo(dtype).min, np.iinfo(dtype).max
                )
            ...
            print(
                f"File for {inp} is missing, thus mix will be created without this stem."
            )
    ...
    data = np.zeros(
        (len(STEMS), 1, max_samples, n_channels), dtype=np.float32
    )  # (1 + S)xB=1xTxC
    stems = {}
    for k, inp in enumerate(STEMS):
    ...
    if baseline_sum == False:
        new_samples = (1 + max_samples // config["KERNEL_SIZE_ENCODER"]) * config[
            "KERNEL_SIZE_ENCODER"
        ]
    ...
    if baseline_sum == False:
            ...
            for k, out in enumerate(audio_output_save):
                ...
                if max_samplingrate not in accepted_sampling_rates:
                    audio = librosa.resample(
                        audio, SR, max_samplingrate, res_type="kaiser_best"
                    )
        ...
        if max_samplingrate not in accepted_sampling_rates:
            audio_out = librosa.resample(
                audio_out.T, SR, max_samplingrate, res_type="kaiser_best"
            ).T
