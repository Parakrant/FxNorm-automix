...
from common_dataprocessing import create_dataset_mixing, create_minibatch_mixing
from common_datatypes import DataType
from common_miscellaneous import (
    uprint,
    recursive_getattr,
    get_process_memory,
    compute_stft,
)
from common_miscellaneous import td_length_from_fd
from common_supernet import SuperNet
...
parser.add_argument(
    ...
    required=False,
    help=(
        "Suffix that is appended to the folder in `./Results`. "
        "If none is provided then it is derived from the config-file name"
    ),
)

parser.add_argument(
    "--results-folder", type=str, help="Results folder of the experiment."
)
...
tee = subprocess.Popen(
    ["tee", os.path.join(results_folder, "training.log")], stdin=subprocess.PIPE
)
...
if "CUDA_VISIBLE_DEVICES" in os.environ:
    ...
    ngpus = subprocess.run(["nvidia-smi", "-L"], capture_output=True).stdout.count(
        b"\n"
    )
...
if Net.input_type == DataType.TIME_SAMPLES:
    ...
    seq_length_td = td_length_from_fd(
        config["TRAINING_SEQ_LENGTH"],
...
uprint(
    f"Maximum validation length is "
...
if config["DEBUG"] == False:
    ...
    if len(data_valid) > 0:
        ...
        def compute_baselines(i):
            ...
            min_size -= (
                min_size % config["HOP_LENGTH"]
            )  # mimic STFT truncation behavior
            ...
            l2_valid_baseline_mix_time[i][src] = np.mean(
                np.square(samples[src] - mix_t)
            )

            for src, inp in zip(config["OUTPUTS"][1:], config["INPUTS"]):
                l1_valid_baseline_mix_freq[i][src] = np.mean(
                    np.abs(stft[src] - stft[inp])
                )
                l2_valid_baseline_mix_freq[i][src] = np.mean(
                    np.square(stft[src] - stft[inp])
                )
                l1_valid_baseline_mix_time[i][src] = np.mean(
                    np.abs(samples[src] - samples[inp])
                )
                l2_valid_baseline_mix_time[i][src] = np.mean(
                    np.square(samples[src] - samples[inp])
                )
        ...
        for src in config["OUTPUTS"]:
            ...
            with open(
                os.path.join(results_folder, "VALID_ERROR_L2_" + src + ".txt"), "a"
            ) as f:
                f.write(
                    " ".join(
                        [
                            f"{_+1.:6.3f}"
                            for _ in np.arange(len(data_valid), dtype=np.float32)
                        ]
                    )
                )
                f.write("\n")
                f.write(
                    " ".join(
                        [
                            f"{l2_valid_baseline_mix_time[_][src]:6.3f}"
                            for _ in range(len(data_valid))
                        ]
                    )
                )
        ...
        for src in config["OUTPUTS"]:
            ...
            with open(
                ...
                f.write(
                    " ".join(
                        [
                            f"{l2_valid_baseline_mix_freq[_][src]:6.3f}"
                            for _ in range(len(data_valid))
                        ]
                    )
                )
    ...
    if config["CALCULATE_STATISTICS"] == True:
        ...
        input_offset, input_scale, num_samples = 0.0, 0.0, 0
    ...
    while True:
        ...
        if is_sentinel(idx_songs):
            ...
            new_idx = np.concatenate(
                [
                    np.random.permutation(len(data_train))
            ...
            for i in range(
                len(all_idx_songs[config["OUTPUTS"][0]]) // config["BATCH_SIZE"]
            ):
                _idx_songs = {}
                for src in config["OUTPUTS"]:
                    _idx_songs[src] = all_idx_songs[src][
        ...
        np_inp, np_tar = create_minibatch_mixing(
            data=data_train,
            sources=config["SOURCES"],
...
def train_network(my_src):
    ...
    device_ids = [
        (_ + my_src_idx * ngpus_per_src) % ngpus for _ in range(0, ngpus_per_src)
    ]
    ...
    np.savez(
        results_folder + "/training_params.npz",
        N_CHANNELS=config["N_CHANNELS"],
        N_STEMS=len(config["INPUTS"]),
    ...
    try:
        torch.jit.save(
            ...
            results_folder + "/net_" + pretty_my_src + ".jit.dump",
        )
    ...
    if my_src == config["TARGETS"][-1]:
        ...
        for name, param in net.named_parameters():
            uprint(
                f"\t{name:30s}\t{str(list(param.shape)):25s}\t{np.prod(param.shape)}"
            )
    ...
    if args.weight_initialization is not None:
        if os.path.isfile(args.weight_initialization[my_src_idx]):
            uprint(
                f"Loading weights for {pretty_my_src} from {args.weight_initialization[my_src_idx]}"
            )
            if config["PRETRAIN_FRONT_END"]:
                pretrained_dict = torch.load(
                    args.weight_initialization[my_src_idx], map_location="cpu"
                )
            ...
            uprint(
                f"Could not find weight initialization file {args.weight_initialization[my_src_idx]}"
            )
            raise RuntimeError(
                f"Could not find weight initialization file {args.weight_initialization[my_src_idx]}"
            )
    ...
    learning_rate = np.concatenate(
        [l * np.ones((n,)) for n, l in config["LEARNING_RATES"]]
    )
    if config["L2_REGULARIZATION"] is None:
        optimizer = optim.Adam(
            net.parameters(), lr=learning_rate[0], amsgrad=config["AMSGRAD"]
        )
    else:
        optimizer = optim.Adam(
            net.parameters(),
    ...
    for key in config["VALID_LOSSES"]:
        for idx, repl in enumerate(
            nn.parallel.replicate(config["VALID_LOSSES"][key], device_ids)
        ):
    ...
    def parallel_perform_valid(data_indices, replicas):
        ...
        val_data = (
            []
        )  # will contain the validation data, each with different sequence length
        for i in data_indices:
            # create input/target
            # `len(targets) + len(stems)` stands for the mixture and S stems
            data = np.zeros(
                (
                    len(config["OUTPUTS"]) + len(config["INPUTS"]),
                    1,
                    valid_sample_length[i],
                    config["N_CHANNELS"],
                ),
                dtype=np.float32,
            )  # (1 + S)xB=1xTxC
        ...
        with torch.no_grad():
            ...
            # forward pass
            val_outputs = nn.parallel.parallel_apply(
                replicas[: len(val_data)], list(zip(val_data, val_losses))
            )
    ...
    uprint(
        f'Starting training with {config["NUM_MINIBATCHES_PER_EPOCH"]} '
    ...
    if config["GRAD_CLIP_MAX_NORM"] is None:
        ...
        grad_history = np.zeros(
            config["NUM_EPOCHS"] * config["NUM_MINIBATCHES_PER_EPOCH"], dtype=np.float32
        )
        grad_history_idx = 0
    ...
    for epoch in range(config["NUM_EPOCHS"]):
        ...
        if epoch in config["SAVE_NET_AT_EPOCHS"]:
            torch.save(
                net.state_dict(),
                results_folder
                + f"/current_model_for_{pretty_my_src}_after{str(epoch)}epochs.params",
            )
        ...
        for minibatch in range(config["NUM_MINIBATCHES_PER_EPOCH"]):
            ...
            loss_scattered = [
                {"train_loss": _}
                for _ in nn.parallel.replicate(
            ...
            outputs = nn.parallel.parallel_apply(
                replicas, list(zip(data_scattered, loss_scattered))
            )
            ...
            if config["GRAD_CLIP_MAX_NORM"] is None:
                # use adaptive scheme
                grad_history[grad_history_idx] = _get_grad_norm(
                    net.parameters(), config["GRAD_CLIP_NORM_TYPE"]
                )
                grad_history_idx += 1

                _max_norm = torch.maximum(
                    _max_norm,
        ...
        for i in range(0, len(data_valid), len(device_ids)):
            parallel_perform_valid(
                list(range(i, min(i + len(device_ids), len(data_valid)))), replicas
            )
        ...
        if config["TENSORBOARD"]:
            ...
            if config["TENSORBOARD"] == "complete":
                for tag, parm in net.named_parameters():
                    writer_loss.add_histogram(
                        tag, parm.grad.data.cpu().numpy(), state["epoch"]
                    )

        # plot training error curve (to jpeg image)
        if np.any(
            train_loss[: epoch + 1] <= 0
        ):  # check for non-positive values (e.g., due to using `SDRLoss`)
        ...
        plt.savefig(
            results_folder + "/loss_" + pretty_my_src + ".jpeg",
            bbox_inches="tight",
            dpi=300,
        )
        plt.clf()
        ...
        if len(data_valid) > 0:
            ...
            for key in valid_loss:
                with open(
                    os.path.join(
                        results_folder, f"VALID_ERROR_{key.upper()}_{pretty_my_src}.txt"
                    ),
                    "a",
                ) as f:
            ...
            fpartial = os.path.join(
                results_folder, f"best_model_for_{pretty_my_src}_valid_"
            )
            for key in valid_loss:
                ...
                if config["TENSORBOARD"]:
                    writer_loss.add_scalar(
                        "validation_loss/" + key, mean_loss_this_epoch, epoch
                    )
                    writer_loss.flush()
                ...
                if mean_loss_this_epoch < best_valid_mean[key]:
                    ...
                    if config["QUANTIZATION_OP"] is not None:
                        torch.save(
                            {
                                _.replace("_float32", ""): recursive_getattr(
                                    net, _.replace("_float32", "")
                                )
                                for _ in net.state_dict()
                            },
                            fpartial + key + "_mean.quant.params",
                        )
                if median_loss_this_epoch < best_valid_median[key]:
                    ...
                    if config["QUANTIZATION_OP"] is not None:
                        torch.save(
                            ...
                            fpartial + key + "_median.quant.params",
                        )
        ...
        if config["L2_REGULARIZATION"] is None:
            ...
            weight_decay = torch.norm(
                torch.stack([torch.norm(p.detach()) for p in parameters])
            )
        ...
        if len(data_valid) > 0:
            ...
            for key in valid_loss:
                out_message += (
                    f"{key} mean: {np.mean(valid_loss[key]):.6f}, "
...
for idx in range(config["NUM_DATAPROVIDING_PROCESSES"]):
    ...
    uprint(
        f"Starting data providing process {idx} with random seed {seed1} and {seed2}"
    )
    fill_queues_processes.append(
        Process(
            target=fill_queues,
            args=(
                seed1,
...
for src in config["TARGETS"]:
    train_network_processes[src] = Process(target=train_network, args=(src,))
