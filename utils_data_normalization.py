...
def running_mean_std(x, N):
    ...
    #     std = np.sqrt(((cumsum[N:] - cumsum[:-N]) - mean)**2 / float(N))
...
def lufs_normalize(x, sr, lufs, log=True):
    ...
    if log:
        print(
            ...
            loudness,
            " max value: ",
            np.max(np.abs(loudness_normalized_audio)),
        )
...
def get_eq_matching(
    ...
    if max_db > min_db:
        ...
        audio_D = compute_stft(
            np.expand_dims(audio_t, 1),
            hop_length,
            n_fft,
...
def get_SPS(x, n_fft=2048, hop_length=1024, smooth=False, frames=False):
    ...
    audio_D = compute_stft(x, hop_length, n_fft, np.sqrt(np.hanning(n_fft + 1)[:-1]))
    ...
    phi = (
        2
        * (np.abs(audio_D_l * np.conj(audio_D_r)))
        / (np.abs(audio_D_l) ** 2 + np.abs(audio_D_r) ** 2)
    )
...
def get_panning_matching(
    ...
    sps_mean_, phi_mean_, _, _ = get_SPS(
        audio_t, n_fft=n_fft, hop_length=hop_length, smooth=True
    )

    side = get_mean_side(sps_mean_, sr=sr, n_fft=n_fft)

    # This panning initial offset shows improvement when re-panning
    if side > 0:
        alpha = 0.7
    else:
        alpha = 0.3
    ...
    sps_mean_, phi_mean, sps_frames, phi_frames = get_SPS(
        audio_t_, n_fft=n_fft, hop_length=hop_length, smooth=True, frames=frames
    )
    ...
    audio_new_D = compute_stft(audio_t_, hop_length, n_fft, window)
    ...
    audio_new_D_ = audio_new_D_ * (
        np.cos(audio_new_D_phase) + np.sin(audio_new_D_phase) * 1j
    )

    audio_new_t = compute_istft(audio_new_D_, hop_length, window)
...
def get_mean_peak(audio, sr=44100, true_peak=False, n_mels=128, percentile=75):
    ...
    for ch in range(audio_.shape[-1]):
        ...
        onset_func = aubio.onset(
            "hfc", buf_size=window_size, hop_size=hop_size, samplerate=sr
        )
        ...
        if onset_times:
            for i, p in enumerate(onset_times[:-1]):
                samples.append(
                    onset_times[i]
                    + np.argmax(np.abs(x[onset_times[i] : onset_times[i + 1]]))
                )
...
def compress(processor, audio, sr, th, ratio, attack, release):
    ...
    if np.max(np.abs(output)) >= 1.0:
        print("Possible clipped samples in output of compressor.")
        output = np.clip(output, -1.0, 1.0)

    return output


def get_comp_matching(
    audio,
    ref_peak,
    ref_std,
    ratio,
    ...
    if max_db > min_db:
        ...
        peak, std = get_mean_peak(
            x, sr, n_mels=n_mels, true_peak=true_peak, percentile=percentile
        )

        print("input ", "%0.2f" % peak, "%0.2f" % std)
        print("ref ", "%0.2f" % ref_peak, "%0.2f" % ref_std)

        #         print(peak, std, ref_peak, ref_std)
        ...
        #     DownwardCompress
            ...
            for rt in ratios:
                for th in ths:
                    ...
                    peak, std = get_mean_peak(
                        y, sr, n_mels=n_mels, true_peak=true_peak, percentile=percentile
                    )
                    print(rt, th, "%0.2f" % peak, "%0.2f" % std)
                    #                     if peak > (ref_peak - ref_std) and peak < (ref_peak + ref_std):
        ...
        #      Upward Expand
            ...
            if expander:
                ...
                print("expand")
                ratios = np.linspace(ratio, max_ratio, max_ratio - ratio + 1)
                #             ths = np.linspace(-1, min_th+10, 2*np.abs(min_th)-1-20)[::-1]
...
def get_reverb_send(
    ...
    if impulse_responses:
        ...
        reverb = AlgorithmicReverb(sample_rate=sr, parameters=rv_parameters)

    reverb.randomize()

    fxchain = AugmentationChain(
        [(eq, rv_prob, False), (reverb, eq_prob, False)],
        shuffle=shuffle,
        parallel=parallel,
    )
...
def compute_loudness_features(args_):
    ...
    print(f"Loudness features for mixture {idx}...")
    ...
    #     loudness_['d_lufs'].append(np.abs(loudness_tar-loudness_out))
    #     loudness_['d_peak'].append(np.abs(peak_tar_db-peak_out_db))

    loudness_["d_lufs"].append(
        sklearn.metrics.mean_absolute_percentage_error([loudness_tar], [loudness_out])
    )
    loudness_["d_peak"].append(
        sklearn.metrics.mean_absolute_percentage_error([peak_tar_db], [peak_out_db])
    )
...
def compute_spectral_features(args_):
    ...
    spec_out_ = compute_stft(
        audio_out_, hop_length, fft_size, np.sqrt(np.hanning(fft_size + 1)[:-1])
    )
    spec_out_ = np.transpose(spec_out_, axes=[1, -1, 0])
    spec_out_ = np.abs(spec_out_)

    spec_tar_ = compute_stft(
        audio_tar_, hop_length, fft_size, np.sqrt(np.hanning(fft_size + 1)[:-1])
    )
    ...
    print(f"Spectral features for mixture {idx}...")
    ...
    for ch in range(channels):
        ...
        tar_sc = librosa.feature.spectral_centroid(
            y=None, sr=sr, S=tar, n_fft=fft_size, hop_length=hop_length
        )

        out_sc = librosa.feature.spectral_centroid(
            y=None, sr=sr, S=out, n_fft=fft_size, hop_length=hop_length
        )

        tar_bw = librosa.feature.spectral_bandwidth(
            y=None,
            sr=sr,
            S=tar,
            n_fft=fft_size,
        ...
        out_bw = librosa.feature.spectral_bandwidth(
            y=None,
            sr=sr,
            S=out,
            n_fft=fft_size,
        ...
        tar_ct = librosa.feature.spectral_contrast(
            y=None,
            sr=sr,
            S=tar,
            n_fft=fft_size,
        ...
        out_ct = librosa.feature.spectral_contrast(
            y=None,
            sr=sr,
            S=out,
            n_fft=fft_size,
        ...
        tar_ro = librosa.feature.spectral_rolloff(
            y=None,
            sr=sr,
            S=tar,
            n_fft=fft_size,
        ...
        out_ro = librosa.feature.spectral_rolloff(
            y=None,
            sr=sr,
            S=out,
            n_fft=fft_size,
        ...
        tar_ft = librosa.feature.spectral_flatness(
            y=None, S=tar, n_fft=fft_size, hop_length=hop_length, amin=1e-10, power=2.0
        )

        out_ft = librosa.feature.spectral_flatness(
            y=None, S=out, n_fft=fft_size, hop_length=hop_length, amin=1e-10, power=2.0
        )

        #         print(idx, 'sc', np.max(tar_sc), np.min(tar_sc))
        #         print(idx, 'bw', np.max(tar_bw), np.min(tar_bw))
        #         print(idx, 'ct', np.max(tar_ct), np.min(tar_ct))
        #         print(idx, 'ro', np.max(tar_ro), np.min(tar_ro))
        #         print(idx, 'ft', np.max(tar_ft), np.min(tar_ft))
        ...
        #         assert np.isnan(std_sc_tar).any() == False, f'NAN values std_sc_tar {idx}'
        assert np.isnan(mean_sc_out).any() == False, f"NAN values mean_sc_out {idx}"
        #         assert np.isnan(std_sc_out).any() == False, f'NAN values std_sc_out {idx}'
        ...
        #         assert np.isnan(std_bw_tar).any() == False, f'NAN values tar std bw {idx}'
        assert np.isnan(mean_bw_out).any() == False, f"NAN values out mean bw {idx}"
        #         assert np.isnan(std_bw_out).any() == False, f'NAN values out std bw {idx}'

        mean_ct_tar, std_ct_tar = get_running_stats(
            tar_ct.T, list(range(tar_ct.shape[0])), N=N
        )
        mean_ct_out, std_ct_out = get_running_stats(
            out_ct.T, list(range(out_ct.shape[0])), N=N
        )

        assert np.isnan(mean_ct_tar).any() == False, f"NAN values tar mean ct {idx}"
        #         assert np.isnan(std_ct_tar).any() == False, f'NAN values tar std ct {idx}'
        assert np.isnan(mean_ct_out).any() == False, f"NAN values out mean ct {idx}"
        #         assert np.isnan(std_ct_out).any() == False, f'NAN values out std ct {idx}'
        ...
        #         assert np.isnan(std_ro_tar).any() == False, f'NAN values tar std ro {idx}'
        ...
        mean_ft_tar, std_ft_tar = get_running_stats(
            tar_ft.T, [0], N=800
        )  # gives very high numbers due to N (80) value
        mean_ft_out, std_ft_out = get_running_stats(out_ft.T, [0], N=800)

        mape_mean_sc = sklearn.metrics.mean_absolute_percentage_error(
            mean_sc_tar[0], mean_sc_out[0]
        )
        #         mape_std_sc = sklearn.metrics.mean_absolute_percentage_error(std_sc_tar[0], std_sc_out[0])

        mape_mean_bw = sklearn.metrics.mean_absolute_percentage_error(
            mean_bw_tar[0], mean_bw_out[0]
        )
        #         mape_std_bw = sklearn.metrics.mean_absolute_percentage_error(std_bw_tar[0], std_bw_out[0])

        mape_mean_ct_l = sklearn.metrics.mean_absolute_percentage_error(
            mean_ct_tar[0], mean_ct_out[0]
        )
        #         mape_std_ct_l = sklearn.metrics.mean_absolute_percentage_error(std_ct_tar[0], std_ct_out[0])

        mape_mean_ct_m = sklearn.metrics.mean_absolute_percentage_error(
            np.mean(mean_ct_tar[1:4], axis=0), np.mean(mean_ct_out[1:4], axis=0)
        )
        #         mape_std_ct_m = sklearn.metrics.mean_absolute_percentage_error(np.mean(std_ct_tar[1:4], axis=0),
        #                                                                         np.mean(std_ct_out[1:4], axis=0))

        mape_mean_ct_h = sklearn.metrics.mean_absolute_percentage_error(
            mean_ct_tar[-1], mean_ct_out[-1]
        )
        #         mape_std_ct_h = sklearn.metrics.mean_absolute_percentage_error(std_ct_tar[-1], std_ct_out[-1])

        mape_mean_ro = sklearn.metrics.mean_absolute_percentage_error(
            mean_ro_tar[0], mean_ro_out[0]
        )
        #         mape_std_ro = sklearn.metrics.mean_absolute_percentage_error(std_ro_tar[0], std_ro_out[0])

        mape_mean_ft = sklearn.metrics.mean_absolute_percentage_error(
            mean_ft_tar[0], mean_ft_out[0]
        )
        #         mape_std_ft = sklearn.metrics.mean_absolute_percentage_error(std_ft_tar[0], std_ft_out[0])

        centroid_mean_.append(mape_mean_sc)
        #         centroid_std_.append(mape_std_sc)
        bandwidth_mean_.append(mape_mean_bw)
        #         bandwidth_std_.append(mape_std_bw)
        contrast_l_mean_.append(mape_mean_ct_l)
        #         contrast_l_std_.append(mape_std_ct_l)
        contrast_m_mean_.append(mape_mean_ct_m)
        #         contrast_m_std_.append(mape_std_ct_m)
        contrast_h_mean_.append(mape_mean_ct_h)
        #         contrast_h_std_.append(mape_std_ct_h)
        rolloff_mean_.append(mape_mean_ro)
        #         rolloff_std_.append(mape_std_ro)
        flatness_mean_.append(mape_mean_ft)

    spectral_["centroid_mean"].append(np.mean(centroid_mean_))
    #     spectral_['centroid_std'].append(np.mean(centroid_std_))

    spectral_["bandwidth_mean"].append(np.mean(bandwidth_mean_))
    #     spectral_['bandwidth_std'].append(np.mean(bandwidth_std_))

    spectral_["contrast_l_mean"].append(np.mean(contrast_l_mean_))
    #     spectral_['contrast_l_std'].append(np.mean(contrast_l_std_))

    spectral_["contrast_m_mean"].append(np.mean(contrast_m_mean_))
    #     spectral_['contrast_m_std'].append(np.mean(contrast_m_std_))
    ...
    spectral_["mape_mean"].append(
        np.mean(
            [
                np.mean(centroid_mean_),
    ...
    #     spectral_['mape_std'].append(np.mean([np.mean(centroid_std_),
    #                                       np.mean(bandwidth_std_),
    #                                       np.mean(contrast_l_std_),
    #                                       np.mean(contrast_m_std_),
    #                                       np.mean(contrast_h_std_),
    #                                       np.mean(rolloff_std_),
    #                                      ]))
...
def compute_panning_features(args_):
    ...
    print(f"Panning features for mixture {idx}...")
    ...
    _, _, sps_frames_tar, _ = get_SPS(
        audio_tar_, n_fft=fft_size, hop_length=hop_length, smooth=True, frames=True
    )

    _, _, sps_frames_out, _ = get_SPS(
        audio_out_, n_fft=fft_size, hop_length=hop_length, smooth=True, frames=True
    )

    p_rms_tar = get_panning_rms(sps_frames_tar, freqs=freqs, sr=sr, n_fft=fft_size)

    p_rms_out = get_panning_rms(sps_frames_out, freqs=freqs, sr=sr, n_fft=fft_size)
    ...
    panning_["P_t_mean"] = [
        sklearn.metrics.mean_absolute_percentage_error(mean_tar[0], mean_out[0])
    ]
    #     panning_['P_t_std'] = [sklearn.metrics.mean_absolute_percentage_error(std_tar[0] + eps, std_out[0])]
    panning_["P_l_mean"] = [
        sklearn.metrics.mean_absolute_percentage_error(mean_tar[1], mean_out[1])
    ]
    #     panning_['P_l_std'] = [sklearn.metrics.mean_absolute_percentage_error(std_tar[1] + eps, std_out[1])]
    panning_["P_m_mean"] = [
        sklearn.metrics.mean_absolute_percentage_error(mean_tar[2], mean_out[2])
    ]
    #     panning_['P_m_std'] = [sklearn.metrics.mean_absolute_percentage_error(std_tar[2] + eps, std_out[2])]
    panning_["P_h_mean"] = [
        sklearn.metrics.mean_absolute_percentage_error(mean_tar[3], mean_out[3])
    ]
    #     panning_['P_h_std'] = [sklearn.metrics.mean_absolute_percentage_error(std_tar[3] + eps, std_out[3])]
    ...
    #     panning_['mape_std'] = [np.mean([panning_['P_t_std'],
    #                                       panning_['P_l_std'],
    #                                       panning_['P_m_std'],
    #                                       panning_['P_h_std'],
    #                                      ])]
...
def get_rms_dynamic_crest(x, frame_length, hop_length):
    ...
    for ch in range(x.shape[-1]):
        frames = librosa.util.frame(
            x[:, ch], frame_length=frame_length, hop_length=hop_length
        )
        rms_ = []
        dynamic_spread_ = []
        crest_ = []
...
def get_low_freq_weighting(x, sr, n_fft, hop_length, f0=1000):
    ...
    X_low = compute_stft(x_low, hop_length, n_fft, np.sqrt(np.hanning(n_fft + 1)[:-1]))
    X_low = np.transpose(X_low, axes=[1, -1, 0])
    X_low = np.abs(X_low)

    X = compute_stft(x, hop_length, n_fft, np.sqrt(np.hanning(n_fft + 1)[:-1]))
...
def compute_dynamic_features(args_):
    ...
    print(f"Dynamic features for mixture {idx}...")
    ...
    with warnings.catch_warnings():
        ...
        rms_tar, dyn_tar, crest_tar = get_rms_dynamic_crest(
            audio_tar_, fft_size, hop_length
        )
        rms_out, dyn_out, crest_out = get_rms_dynamic_crest(
            audio_out_, fft_size, hop_length
        )

        low_ratio_tar = get_low_freq_weighting(
            audio_tar_, sr, fft_size, hop_length, f0=1000
        )

        low_ratio_out = get_low_freq_weighting(
            audio_out_, sr, fft_size, hop_length, f0=1000
        )
    #         print(idx, np.max(low_ratio_tar), np.max(low_ratio_out))

    N = 40

    eps = 1e-10

    #     print(rms_tar.shape, dyn_tar.shape, crest_tar.shape, low_ratio_tar.shape)
    #     print(idx, 'rms', np.max(rms_tar), np.min(rms_tar))
    #     print(idx, 'dyn', np.max(dyn_tar), np.min(dyn_tar))
    #     print(idx, 'crest', np.max(crest_tar), np.min(crest_tar))
    #     print(idx, 'low_ratio_tar', np.max(low_ratio_tar), np.min(low_ratio_tar))
    ...
    #     rms_tar = db_to_amp(rms_tar)
    #     rms_out = db_to_amp(rms_out)
    #     dyn_tar = db_to_amp(dyn_tar)
    #     dyn_out = db_to_amp(dyn_out)
    ...
    dynamic_["rms_mean"] = [
        sklearn.metrics.mean_absolute_percentage_error(mean_rms_tar, mean_rms_out)
    ]
    #     dynamic_['rms_std'] = [sklearn.metrics.mean_absolute_percentage_error(std_rms_tar, std_rms_out)]
    dynamic_["dyn_mean"] = [
        sklearn.metrics.mean_absolute_percentage_error(mean_dyn_tar, mean_dyn_out)
    ]
    #     dynamic_['dyn_std'] = [sklearn.metrics.mean_absolute_percentage_error(std_dyn_tar, std_dyn_out)]
    dynamic_["crest_mean"] = [
        sklearn.metrics.mean_absolute_percentage_error(mean_crest_tar, mean_crest_out)
    ]
    #     dynamic_['crest_std'] = [sklearn.metrics.mean_absolute_percentage_error(std_crest_tar, std_crest_out)]

    dynamic_["l_ratio_mean_mape"] = [
        sklearn.metrics.mean_absolute_percentage_error(
            mean_low_ratio_tar, mean_low_ratio_out
        )
    ]
    dynamic_["l_ratio_mean_l2"] = [
        sklearn.metrics.mean_squared_error(mean_low_ratio_tar, mean_low_ratio_out)
    ]
    #     dynamic_['l_ratio_std'] = [sklearn.metrics.mean_absolute_percentage_error(std_low_ratio_tar, std_low_ratio_out)]
    ...
    dynamic_["mape_mean"] = [
        np.mean(
            [
                ...
                dynamic_["dyn_mean"],
                dynamic_["crest_mean"],
                #                                       dynamic_['l_ratio_mean'],
            ]
        )
    ]

    #     dynamic_['mape_std'] = [np.mean([dynamic_['rms_std'],
    #                                       dynamic_['dyn_std'],
    #                                       dynamic_['crest_std'],
    # #                                       panning_['l_ratio_std'],
    #                                      ])]
