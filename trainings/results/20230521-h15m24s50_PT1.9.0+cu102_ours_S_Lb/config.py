...
config["INPUTS"] = [
    ...
    "drums_normalized",
    "other_normalized",
]  # Must follow names of normalized wav files. e.g.vocals_normalized.wa
...
from automix.common_networkbuilding_cafx_tdcn_lstm_mix import (
    Net,
...
config["PRETRAIN_FRONT_END"] = True
...
RF, guard = compute_receptive_field(
...
config["LEARNING_RATES"] = [
    (150, config["INITIAL_LEARNING_RATE"]),
    (50, config["INITIAL_LEARNING_RATE"] / 3.0),
    (50, config["INITIAL_LEARNING_RATE"] / 10.0),
    (25, config["INITIAL_LEARNING_RATE"] / 30.0),
    (25, config["INITIAL_LEARNING_RATE"] / 100.0),
    (10, config["INITIAL_LEARNING_RATE"] / 1000.0),
]
...
config["TRAINING_SEQ_LENGTH"] = (10 * min(config["ACCEPTED_SAMPLING_RATES"])) // config[
    "KERNEL_SIZE_ENCODER"
]
config["TRAINING_SEQ_LENGTH"] = (
    config["TRAINING_SEQ_LENGTH"] * config["KERNEL_SIZE_ENCODER"]
)

# Frequency bins that we keep for processing (only up to 16khz to avoid instabilities)
# THIS IS NOT USEFUL FOR CONV-TASNET.
config["MAX_PROCESSED_FREQUENCY"] = np.minimum(
    16000.0, np.min(config["ACCEPTED_SAMPLING_RATES"]) / 2.0
)
config["N_BINS_KEEP"] = int(
    config["MAX_PROCESSED_FREQUENCY"]
    / (np.min(config["ACCEPTED_SAMPLING_RATES"]) / 2.0)
    * config["N_BINS"]
)
...
config["NUM_MINIBATCHES_FOR_STATISTICS_ESTIMATION"] = 10
...
config["NUM_MINIBATCHES_PER_EPOCH"] = 16
...
config["BATCHED_TEST"] = False

# Loss function(s) for training (`list`)

f_guard = int(
    np.ceil(
        ((config["GUARD_LEFT"] - (config["FFT_SIZE"] - 1) - 1) / config["HOP_LENGTH"])
        + 1
    )
    + 1
)
...
loss = StereoLoss2(
    Loss(nn.L1Loss(), DataType.STFT_MAGNITUDE, guard_left=f_guard, guard_right=f_guard),
    filter_type=["aw", "lp"],
)
...
config["VALID_LOSSES"]["stereo_loss"] = StereoLoss2(
    Loss(nn.L1Loss(), DataType.STFT_MAGNITUDE, guard_left=f_guard, guard_right=f_guard),
    filter_type=["aw", "lp"],
)
...
config["MAX_VALIDATION_SEQ_LENGTH_TD"] = (
    4 * 60 * np.max(config["ACCEPTED_SAMPLING_RATES"])
)
# TODO: Increase if we have larger GPUs

# Specify folders where the training data is stored
config["DATA_DIR_TRAIN"] = []
config["DATA_DIR_TRAIN"].append(
    ("/home/u571/work/audio_mix/FxNorm-automix/train", False)
)

# Specify folders where the validation data is stored
config["DATA_DIR_VALID"] = []
config["DATA_DIR_VALID"].append(("/home/u571/work/audio_mix/FxNorm-automix/val", False))
