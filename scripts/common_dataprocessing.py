...
def load_wav(file_path, mmap=False, convert_float=False):
    ...
    if convert_float:
        conversion_scale = 1.0 / (1.0 + np.iinfo(samples.dtype).max)
...
def save_wav(file_path, fs, samples, subtype="PCM_16"):
    ...
    # store WAV file
    #     wav.write(file_path, fs, samples)
...
def create_dataset(
    ...
    if mapped_sources and not load_to_memory:
        raise ValueError(
            "Mapping of sources only supported if data is loaded into the memory."
        )
    ...
    if debug:
        data = [
            dict()
            for _x in range(np.minimum(NUM_SAMPLES_SMALL_DATASET, len(directories)))
        ]
    else:
    ...
    for i, d in enumerate(directories):
        ...
        for f in files:
            ...
            if src_name not in sources and src_name not in mapped_sources:
                ...
                data[i][src_name] = functools.partial(
                    generate_data, file_path_or_data=_data
                )
    ...
    uprint(
        f"Finished preparation of dataset. "
...
def create_dataset_mixing(
    ...
    for i, d in enumerate(directories):
        ...
        for f in files:
            ...
            if src_name not in sources and src_name not in mapped_sources:
                ...
                if pad_wrap_samples:
                    _data = (
                        _data[0],
                        np.pad(_data[1], [(pad_wrap_samples, 0), (0, 0)], "wrap"),
                    )
        ...
        data[i]["-".join(_stems_name)] = functools.partial(
            generate_data, file_path_or_data=_data_mix
        )
...
def generate_data(file_path_or_data, random_sample_size=None):
    ...
    if (
        type(file_path_or_data[0]) is not list
    ...
    if random_sample_size is not None:
        ...
        for i, s in enumerate(samples):
            if s.shape[0] < max_length:
                ...
                zeros = np.zeros(
                    (required_padding // 2 + 1, s.shape[1]), dtype=s.dtype, order="F"
                )
        ...
        for i, s in enumerate(samples):
            if idx_start + random_sample_size < s.shape[0]:
                ...
                samples[i] = np.concatenate(
                    [s[idx_start:], s[: random_sample_size - (s.shape[0] - idx_start)]]
                )

    # convert from `int16/int32` to `float32` precision (this will also make a copy)
    for i, s in enumerate(samples):
        conversion_scale = 1.0 / (1.0 + np.iinfo(s.dtype).max)
...
def create_minibatch_mixing(
    ...
    augmenter_padding: Tuple[int],
    augmenter_sources: list,
    batch_size: int,
    n_samples: int,
    n_channels: int,
    idx_songs: dict,
    ...
    #     mix = np.zeros(shape=shp, dtype=np.float32, order='C')
    ...
    def augm(i, n):
        ...
        for k, tag in enumerate(audio_tags):
            if tag in augmenter_sources:
                input_multitracks[k] = augmenter(input_multitracks[k])[
                    pad_left:pad_right
                ]
            else:
    ...
    #         return augmenter(input_multitracks)[pad_left:pad_right]
