...
from common_dataprocessing import (
    create_dataset_mixing,
    create_minibatch_mixing,
    generate_data,
    create_dataset,
)
from common_supernet import SuperNet
from common_losses import StereoLoss, StereoLoss2, Loss
...
from utils_data_normalization import (
    get_panning_matching,
    get_SPS,
    amp_to_db,
    get_reverb_send,
)
from utils_data_normalization import (
    compute_loudness_features,
    compute_spectral_features,
    compute_panning_features,
)
from utils_data_normalization import compute_dynamic_features, print_dict
...
eq_gain = -30.0
EQ_PARAMETERS.add(
    Parameter("low_shelf_gain", eq_gain, "float", minimum=eq_gain, maximum=eq_gain)
)
EQ_PARAMETERS.add(
    Parameter("low_shelf_freq", 600.0, "float", minimum=500.0, maximum=700.0)
)
EQ_PARAMETERS.add(
    Parameter("high_shelf_gain", eq_gain, "float", minimum=eq_gain, maximum=eq_gain)
)
EQ_PARAMETERS.add(
    Parameter("high_shelf_freq", 8500.0, "float", minimum=7000.0, maximum=10000.0)
)
...
PRE_REVERB_PARAMETERS.add(
    Parameter("pre_delay", 0, "int", units="ms", minimum=0, maximum=0)
)
        ...
        if max_db > MIN_DB:
            for effect in effects:
                if effect == "eq":
                    ...
                    for ch in range(audio_track.shape[1]):
                        audio_comp_matched = get_comp_matching(
                            output_audio[:, ch],
                    ...
                    if PANNING_FRAMES:
                        ...
                        R = int(
                            2 * (features_mean[effect][src].shape[0] - 1) / FFT_SIZE_PAN
                        )
                    ...
                    audio_pan_matched = get_panning_matching(
                        output_audio,
                    ...
                    output_audio = lufs_normalize(
                        output_audio, SR, features_mean[effect][src]
                    )
                    ...
                    if src in ["bass", "drums"]:
                        ...
                        audio_reverb_send = get_reverb_send(
                            output_audio,
                            EQ_PARAMETERS,
                            REVERB_PARAMETERS,
                            IR,
                            bands=bands,
                            eq_prob=EQ_PROBABILITY,
                            rv_prob=REVERB_PROBABILITY,
                    ...
                    if src in ["bass", "drums"]:
                        ...
                        audio_reverb_send = get_reverb_send(
                            output_audio,
                            EQ_PARAMETERS,
                            PRE_REVERB_PARAMETERS,
                            PRE_IR,
                            bands=bands,
                            eq_prob=PRE_EQ_PROBABILITY,
                            rv_prob=PRE_REVERB_PROBABILITY,
        ...
        for key in STEMS:
            if effect == "eq":
                if key in ["other", "vocals"]:
                    ...
                    f = 151
                feature_dict_[effect][key] = scipy.signal.savgol_filter(
                    feature_dict_[effect][key], f, 1, mode="mirror"
                )
            elif effect == "panning":
                feature_dict_[effect][key] = scipy.signal.savgol_filter(
                    feature_dict_[effect][key], 501, 1, mode="mirror"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input-folder", help="Audio folder to mix", type=str, required=True
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to training parameters and audio settings",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to folder with impulse responses",
    )

    parser.add_argument(
        "--pre-impulse-responses",
        type=str,
        help="Path to folder with impulse responses for pre-reverb",
        default=None,
    )

    parser.add_argument(
        "--nets", type=str, help="path to net.dump or net.jit.dump files", default=None
    )
    ...
    parser.add_argument(
        "--output-folder", type=str, help="Name of output folder", required=True
    )

    parser.add_argument(
        "--output-prefix", type=str, help="Name of output prefix", required=True
    )
    ...
    if ir_path["prereverb"]:
        ...
        PRE_REVERB_PARAMETERS.add(
            Parameter("index", 0, "int", minimum=0, maximum=len(PRE_IR))
        )
    else:
    ...
    mixture_outputs = {
        key: [] for key in [DataType.STFT_MAGNITUDE, DataType.TIME_SAMPLES]
    }
    mixture_targets = {
        key: [] for key in [DataType.STFT_MAGNITUDE, DataType.TIME_SAMPLES]
    }
    ...
    for i in range(len_data):
        ...
        if baseline_sum == False:
            new_samples = (1 + samples // config["KERNEL_SIZE_ENCODER"]) * config[
                "KERNEL_SIZE_ENCODER"
            ]
        ...
        if baseline_sum == False:
            ...
            with torch.no_grad():
                ...
                test_tar, test_out, test_losses_ = super_net.evaluate(
                    test_data, config["TEST_LOSSES"]
                )
                ...
                for k, out in enumerate(audio_output_save):
                    ...
                    if SAVE_OUTPUT_WAV:
                        ...
                        output_name = os.path.join(
                            output_name, f"{output_prefix}_mixture.wav"
                        )
            ...
            if SAVE_OUTPUT_WAV:
                ...
                os.makedirs(output_name, exist_ok=True)
                output_name = os.path.join(
                    output_name, f"{output_prefix}_baseline_mixture.wav"
                )
    ...
    if COMPUTE_LOUDNESS:
        ...
        for i, audio_ in enumerate(
            zip(
                mixture_outputs[DataType.TIME_SAMPLES],
                mixture_targets[DataType.TIME_SAMPLES],
            )
        ):
            _func_args.append((audio_[0], audio_[1], i, SR))
    ...
    if COMPUTE_PANNING:
        ...
        for i, audio_ in enumerate(
            ...
            _func_args.append((audio_[0], audio_[1], i, SR, n_fft, hop_length))
    ...
    if COMPUTE_DYNAMIC:
        ...
        for i, audio_ in enumerate(
            ...
            _func_args.append(
                (audio_[0], audio_[1], i, SR, config["FFT_SIZE"], config["HOP_LENGTH"])
            )
