...
from common_datatypes import DataType, time_domain_to_spectral, remove_guards
...
class Loss(nn.Module):
    ...
    def forward(self, out: dict, tar: dict):
        ...
        return self._loss(
            remove_guards(
                out[self.data_type], self.guard_left, self.guard_right, self.data_type
            ),
            remove_guards(
                tar[self.data_type], self.guard_left, self.guard_right, self.data_type
            ),
        ).mul_(self._scale)
...
class FIRFilterLoss(Loss):
    def __init__(
        ...
        for fir_type in self.filter_type:
            self.weights.append(
                self.get_weights(fir_type, self.coef, self.fs, self.ntaps)
            )
    ...
    def process_filter(self, inp, weights):
        ...
        for i in range(n_sources):
            ...
            for j in range(n_channels):
                ...
                ch__ = torch.nn.functional.conv1d(
                    ch_.clone(), weights, padding=self.ntaps // 2
                )
                ch.append(ch__)
            ch = torch.hstack(ch)
            inp[i, ...] = ch[..., :n_samples]
    ...
    def forward(self, out: dict, tar: dict):
        ...
        for i in range(len(self.filter_type)):
            out[self.data_type] = self.process_filter(
                out[self.data_type], self.__getattr__(f"_weights{i}")
            )
            tar[self.data_type] = self.process_filter(
                tar[self.data_type], self.__getattr__(f"_weights{i}")
            )

        if self._loss.data_type in [
            DataType.STFT_MAGNITUDE,
            DataType.STFT_MAGNITUDEPHASE,
            DataType.STFT_COMPLEX,
        ]:
            out[self._loss.data_type] = time_domain_to_spectral(
                out[self.data_type],
            ...
            tar[self._loss.data_type] = time_domain_to_spectral(
                tar[self.data_type],
                self._loss.data_type,
                self._window,
                self._fft_size,
...
class StereoLoss(Loss):
    ...
    def __init__(
        ...
        fft_size=4096,
        hop_length=1024,
        stft_window=np.sqrt(np.hanning(4096 + 1)[:-1]),
    ):
        """
        ...
        if self.filter_type:
            self.fir = FIRFilterLoss(
                loss, filter_type=filter_type, coef=coef, fs=fs, ntaps=ntaps, amp=amp
            )
    ...
    def sum_diff(self, x):
        ...
        diff_ = torch.stack([channels[0], -1.0 * channels[1]], dim=0).sum(
            dim=0, keepdim=False
        )
    ...
    def compute_spectral(self, x, datatype, win, fft, hop):
        ...
        X = time_domain_to_spectral(x, datatype, win, fft, hop)
    ...
    def forward(self, out: dict, tar: dict):
        ...
        if self.filter_type:
            for i in range(len(self.filter_type)):
                out_ = self.fir.process_filter(
                    out_, self.fir.__getattr__(f"_weights{i}")
                )
                tar_ = self.fir.process_filter(
                    tar_, self.fir.__getattr__(f"_weights{i}")
                )

        out_sum, out_diff = self.sum_diff(out_)
        tar_sum, tar_diff = self.sum_diff(tar_)

        out_sum_mag = self.compute_spectral(
            out_sum,
            self._loss.data_type,
            self._window,
            self._fft_size,
        ...
        out_diff_mag = self.compute_spectral(
            out_diff,
            self._loss.data_type,
            self._window,
            self._fft_size,
        ...
        tar_sum_mag = self.compute_spectral(
            tar_sum,
            self._loss.data_type,
            self._window,
            self._fft_size,
        ...
        tar_diff_mag = self.compute_spectral(
            tar_diff,
            self._loss.data_type,
            self._window,
            self._fft_size,
