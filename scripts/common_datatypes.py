...
from common_miscellaneous import valid_length
...
def spectral_to_time_domain(
    spectrogram, spectrogram_type, window, fft_size, hop_length
):
    """
    ...
    """
    assert spectrogram_type in [DataType.STFT_MAGNITUDEPHASE, DataType.STFT_COMPLEX]
...
def time_domain_to_spectral(
    time_signal, spectrogram_type, window, fft_size, hop_length
):
    """
...
def remove_guards(
    inp: torch.Tensor, guard_left: int, guard_right: int, input_type: DataType
) -> torch.Tensor:
    """
    Remove guards.
...
def unfold(
    ...
    assert (
        hop_size > 0
    ), "`hop_size` for unfold not positive - `guard_left/right` are too large"
    ...
    necessary_padding = valid_length(
        input_size=guard_left + original_length + guard_right,
        kernel_size=window_size,
        stride=hop_size,
        padding=0,
        dilation=1,
    ) - (guard_left + original_length + guard_right)
    ...
    inp = torch.nn.functional.pad(
        inp,
        pad=(
            (0, 0) * right_unpadded_dims + (guard_left, necessary_padding + guard_right)
        ),
    )
...
def reconstruct_from_unfold(
    ...
    if input_type == DataType.TIME_SAMPLES:
        assert inp.ndim == 4, (
            "Expecting 4D input of shape "
            "n_targets x batch_size x n_channels x n_samples"
        )
        samples_dim = 3
    elif input_type == DataType.STFT_MAGNITUDE:
        assert inp.ndim == 5, (
            "Expecting 5D input of shape "
    ...
    output = inp.narrow(
        dim=samples_dim, start=guard_left, length=window_size - guard_right - guard_left
    )
    ...
    output = output[
        :, None, ...
    ]  # add the batch dimension to comply with DataType standards
