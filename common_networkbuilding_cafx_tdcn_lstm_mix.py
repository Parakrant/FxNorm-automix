...
from common_supernet import SeparationNet
from common_miscellaneous import pad_to_shape


def compute_receptive_field(
    KERNEL_SIZE_ENCODER, KERNEL_SIZE_TB, N_TB_PER_REPEAT, N_REPEATS, MAX_POOLING
):
...
class SqueezeExcitation(nn.Module):
    ...
    def __init__(self, n_channels, amplifying_ratio) -> None:
        ...
        self.net = nn.Sequential(
            nn.Linear(self.n_channels, n_channels_expanded, bias=True),
            nn.ReLU(),
...
class Net(SeparationNet):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.bottleneck_conv1x1 = nn.Conv1d(
            N_FEATURES_ENCODER, N_FEATURES_SEPARATION_MODULE, 1
        )
        ...
        self.output = nn.Sequential(
            nn.PReLU(),
            nn.Conv1d(N_FEATURES_OUT, n_targets * N_FEATURES_ENCODER, kernel_size=1),
            nn.Softplus(),
        )
        ...
        self.conv_1 = nn.Conv1d(
            in_channels=n_channels * n_stems,
        ...
        self.conv_2 = nn.Sequential(
            nn.Conv1d(
                in_channels=N_FEATURES_ENCODER,
                out_channels=N_FEATURES_ENCODER,
        ...
        self.maxunpool1d = (
            nn.MaxUnpool1d(self.maxpoolsize)
            if self.pretrain
            else nn.Upsample(scale_factor=self.maxpoolsize)
        )

        self.se_block = SqueezeExcitation(
            n_channels=N_FEATURES_ENCODER, amplifying_ratio=self.se_amp_ratio
        )
                ...
                for names in l._all_weights:
                    for name in filter(lambda n: "bias" in n, names):
                        ...
                        if heuristic == "init_1":
                            ...
                            bias.data[n : 2 * n] = torch.log(
                                nn.init.uniform_(bias.data[0:n], Tmin, Tmax - 1)
                            )
    ...
    def forward(self, x):
        ...
        # output [batch_size, n_targets, n_features_encoder, hidden_size]
        masking = masking.view(
            batch_size, self.n_targets, self.n_features_encoder, masking.shape[-1]
        )
